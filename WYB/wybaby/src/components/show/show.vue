<template>
  <div class="show" v-show="bb">
    ...
    <div class="jgxq">
      <div class="jgxq1">
        <div class="jg">{{xiangqing.description}}</div>
        <div class="aixin">
          <img src="../../../img/kong.png" alt v-show="kong" @click="aixin(xiangqing)">
          <img
            v-show="!kong"
            src="../../../img/aixin.png"
            alt
        ...
        <div class="jjg">¥{{xiangqing.price}}</div>
        <div class="oldp">{{xiangqing.oldprice}}</div>
      </div>
    </div>
    <!-- 评论 -->
    <div
      class="pl1"
      style="font-weight:700;width:100%;height:50px;text-align:center;line-height:50px;background:white;border:1px solid gray;"
    >评论区</div>
    ...
    <div class="xiepl">
      <input type="text" class="pl-text" placeholder="写评论..." style="width:300px">
...
export default {
  ...
  methods: {
    ...
    aixin(o) {
      ...
      this.$axios({
        method: "post",
        url: "http://localhost:3000/shoucang",
        data: {
          pic:o.av,
          name:o.description,
          price:o.price
  ...
  mounted() {
    ...
    this.$axios({
      ...
      if (name == "精品推荐") {
        ...
        this.images = array[id].imgs;
        this.shopimages = array[id].shopimgs;
      } else if (name == "新品上架") {
    ...
    this.$axios({
      ...
      for (var k = 0; k < res.data.length; k++) {
        if (this.xiangqing.description == res.data[k].description) {
